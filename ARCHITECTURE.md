...
- **Deploy approach**: Manual. `alexa-skill-model.json` in repo root is the source of truth. When intents change, paste the JSON into the developer console JSON Editor, Save, and Build Skill. API response changes deploy automatically via Vercel (no console update needed).
